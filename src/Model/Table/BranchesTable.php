...
class BranchesTable extends Table
{
    ...
    public function validationDefault(Validator $validator): Validator
    {
        ...
        $validator
            ->scalar('br_name')
            ->allowEmptyString('br_name');

        $validator
            ->scalar('br_abn')
            ->maxLength('br_abn', 24)
            ->allowEmptyString('br_abn');

        $validator
            ->scalar('br_address')
            ->allowEmptyString('br_address');

        $validator
            ->scalar('br_phone')
            ->maxLength('br_phone', 16)
            ->allowEmptyString('br_phone');

        $validator
            ->scalar('br_email')
            ->maxLength('br_email', 64)
            ->allowEmptyString('br_email');

        $validator
            ->scalar('br_currency')
            ->maxLength('br_currency', 16)
            ->allowEmptyString('br_currency');

        $validator
            ->scalar('br_logo')
            ->allowEmptyString('br_logo');

        $validator
            ->allowEmptyString('so_pickupFlag');

        $validator
            ->allowEmptyString('in_locationFlag');
